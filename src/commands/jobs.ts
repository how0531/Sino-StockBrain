...
export async function registerBuiltinHandlers(worker: MinionWorker, engine: BrainEngine): Promise<void> {
  ...
  {
    const { fundamentalsRevenueHandler } = await import('../core/minions/handlers/fundamentals-revenue.ts');
    worker.register('fundamentals-revenue', fundamentalsRevenueHandler);
  ...
  {
    const { moversDetectHandler } = await import('../core/minions/handlers/movers-detect.ts');
    worker.register('movers-detect', moversDetectHandler);
